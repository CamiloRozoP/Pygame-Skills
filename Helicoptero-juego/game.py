...
def main_menu():
    ...
    while menu:
        ...
        controls_1 = message_to_screen("usa W A S D para moverte, SPACE para disparar,", font, 30, black)
        controls_2 = message_to_screen("SHIFT para tirar bombas, and P para poner pausa", font, 30, black)
        if selected == "play":
            play = message_to_screen("JUGAR", font, 75, white)
        else:
            play = message_to_screen("JUGAR", font, 75, black)
        if selected == "quit":
            game_quit = message_to_screen("SALIR", font, 75, white)
        else:
            game_quit = message_to_screen("SALIR", font, 75, black)
        ...
        pygame.display.set_caption("HELICOPTERO corriendo a " + str(int(clock.get_fps())) + " frames por segundo.")
        clock.tick(FPS)
...
def pause():
    ...
    paused_text = message_to_screen("PAUSE", font, 100, black)
...
def game_loop():
    ...
    while not game_exit:
        ...
        while game_over:
            ...
            your_score = message_to_screen("TU PUNTAJE FUE: " + str(score), font, 50, black)
            if game_over_selected == "play again":
                play_again = message_to_screen("JUGAR DE NUEVO", font, 75, white)
            else:
                play_again = message_to_screen("JUGAR DE NUEVO", font, 75, black)
            if game_over_selected == "quit":
                game_quit = message_to_screen("SALIR", font, 75, white)
            else:
                game_quit = message_to_screen("SALIR", font, 75, black)
        ...
        # dibuja al azar globos posicionados aleatoriamente 
        ...
        # deteccion bala enemigo colision
        ...
        #deteccion nave-jugador balas/bombas colisiones 
        ...
        #deteccion bote-jugador balas/bombas colisiones
        ...
        #deteccion jugador-globo balas/bombas colisiones
        ...
        # deteccion jugador-enemigo cohete colision 
        ...
        # deteccion jugador-bote bala colision 
        ...
        # deteccion jugador-bote  colision 
        ...
        # deteccion jugador-nave  colision 
        ...
        # dibuja puntaje
        game_display.blit(message_to_screen("PUNTAJE: {0}".format(score), font, 50, black), (10, 10))

        # dibuja puntaje mas alto
        if score < highscore_int:
            hi_score_message = message_to_screen("PUNTAJE TOP: {0}".format(highscore_int), font, 50, black)
        else:
            highscore_file = open('highscore.dat', "w")
            highscore_file.write(str(score))
            highscore_file.close()
